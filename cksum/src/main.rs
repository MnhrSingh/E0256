// Written by Manohar Lal - 16896 - manoharlsingh@yahoo.com
...
fn crc_rem(r: Vec<u32>,m: i32)->u32{
	...
	for i in 0..8{
		let mut flag: i32=0;
		if(Bit(i)>=(1u32<<31))&&(m<0){
			flag=1;
		}
		for k in 0..31{
			if(m&(1<<k))>0 && (Bit(i)&(1u32<<k))>0{
				flag=1;
			}
		}
		if (flag>0){
			rem=rem^(r[i as usize]);
		}
	}
			...
			match buf.read(&mut byte_arr){
				Ok(len_bytes)=>{
					...
					for &byt in byte_arr[..len_bytes].iter(){
						crc_fin=crc_upd(crc_fin,byt,&crc_tab);
					}
